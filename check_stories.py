...
def main() -> int:
    ...
    check("Static site: overview mirrors Stitch (emerging-green callout)", "is currently classified as an Emerging Green Zone" in idx)
    check("Static site: top-3 momentum widget present", "Top 3 environmental momentum stations" in idx)
    rm = (SITE_DIR / "risk_map.html").read_text(encoding="utf-8")
    check("Static site: risk map shows dossier + GeoJSON export", "Selected Station Dossier" in rm and "stations.geojson" in rm)
    check("Static site: risk map layer chips are honest counts", "Emerging Green Zones" in rm and "Challenge Zones" in rm)
    ...
    check("Static site: station policy checklist present", "Municipal policy checklist" in sa)
    trend = (SITE_DIR / "environmental_trends.html").read_text(encoding="utf-8")
    check("Static site: trends page focus-station pills present", "Focus station" in trend and "DEB-KER11" in trend)
    ...
    check("Static site: audit-protocol header present", "Audit Protocol" in dq)
